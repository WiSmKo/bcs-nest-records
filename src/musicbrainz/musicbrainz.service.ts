...
import { FindUrlsResponse } from './data-interfaces/find-urls-response';
...
export class MusicbrainzService {
    ...
    async findUrls(musicbrainzId: String): Promise<FindUrlsResponse> {
        const queryParams = {
            targetid: musicbrainzId,
        }
        const url = this.buildQuery(queryParams, this.URL);
        this.logger.log(`Searching url: ${url}`);

        const { data } = await lastValueFrom(this.httpService.get<FindUrlsResponse>(url).pipe(
            catchError((error: AxiosError) => {
                this.logger.error('An error occurred making request to Musicbrainz API:', error);
                throw error;
            })
